		...
		<CS.Totalframe>
			<CS.ScreenComponent>
				...
				<Column gap={24}>
					<Row gap={8}>
						<img
							src={checkedItems.service ? FillCheck : Check}
							onClick={() => handleCheckItem("service")}
						/>
					...
					<Row gap={8}>
						<img
							src={checkedItems.personal ? FillCheck : Check}
							onClick={() => handleCheckItem("personal")}
						/>
						<Typography typoSize="B1_semibold" color="Red500">{`필수`}</Typography>
						<S.AgreeText
							onClick={() => navigate("/personalinfotxt")}
						>{`개인정보 수집 및 이용`}</S.AgreeText>
					</Row>
					<Row gap={8}>
						<img
							src={checkedItems.location ? FillCheck : Check}
							onClick={() => handleCheckItem("location")}
						/>
						<Typography typoSize="B1_semibold" color="Red500">{`필수`}</Typography>
