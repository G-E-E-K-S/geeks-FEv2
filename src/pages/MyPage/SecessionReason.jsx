...
const Bottom = styled.div`
	position: fixed;
	bottom: 20px;
	background: linear-gradient(180deg, rgba(255, 255, 255, 0) 0%, #ffffff 25.24%);
		...
		<>
			...
			<Bottom>
				<c.Flex>
					...
					<ChoiceBtn onClick={() => setModalOpen(true)}>{`탈퇴할래요`}</ChoiceBtn>
				</c.Flex>
			</Bottom>
			{modalOpen && (
				<Modal padding={`22px 20px`}>
					...
					<c.Flex>
						<ModalChoiceBtn useMore={true} onClick={() => setModalOpen(false)}>
							{`더 쓸래요`}
						</ModalChoiceBtn>
